...
const NotePage = props => {
	// Созраняем id из url в виде переменной
	const id = props.match.params.id;

	// Запрашиваем хук, передавая значение id в качестве переменной
	const { loading, error, data } = useQuery(GET_NOTE, { variables: { id } });

	// Если данные загружаются, отображается информация о загрузке
	if (loading) {
		return <p>Loading...</p>;
	}

	// Если при получении данных произошел сбой, отображается сообщение об ошибке
	if (error) {
